...
import com.rgy.setcpu.AppAllActivity.AsyncTaskSetModel;
import com.rgy.widget.MyProgressDialog;
import com.rgy.widget.SlidingMenu;

public class AppPowersaveActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_applist_powersave);
		
		view_app_powersave = (RelativeLayout)findViewById(R.id.app_powersave_layout);
		view_app_powersave.setBackgroundColor(0x11000000);
...
//		dialog.show();
		...
		view_app_all.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(AppPowersaveActivity.this,AppAllActivity.class);  
		...
		view_app_default.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(AppPowersaveActivity.this,AppDefaultActivity.class);  
				startActivity(intent);
				finish();
			}
		});
		...
		view_app_performance.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(AppPowersaveActivity.this,AppPerformanceActivity.class);  
				startActivity(intent);
				finish();
			}
		});
		...
		view_switch.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				String customSwitch = myApp.getCustomSwitch();
				if(customSwitch.equals("Stop")){
					Intent intent = new Intent(AppPowersaveActivity.this, CustomService.class);
					startService(intent);
					Toast.makeText(AppPowersaveActivity.this, "自定义模式已启动", Toast.LENGTH_SHORT).show();
					...
					if(smartySwitch.equals("Start")){
						Intent smartyIntent = new Intent(AppPowersaveActivity.this, SmartyService.class);
					...
					Intent intent = new Intent(AppPowersaveActivity.this, CustomService.class);
					stopService(intent);
					Toast.makeText(AppPowersaveActivity.this, "自定义模式已停止", Toast.LENGTH_SHORT).show();
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				//
				Intent intent = new Intent(AppPowersaveActivity.this,AppEachInfoActivity.class);
...
//		new AsyncTaskGetData().execute();
		...
		if(allAppList == null){
			Toast.makeText(AppPowersaveActivity.this, "省电列表为空，您还未添加应用", Toast.LENGTH_SHORT).show();
			return;
		}
		AppListAdapter appListAdapter = new AppListAdapter(AppPowersaveActivity.this, allAppList);
...
//			startActivity(intent);
	...
	class AsyncTaskGetData extends AsyncTask<String, Integer, ArrayList<AppInfo>>{
		...
		protected ArrayList<AppInfo> doInBackground(String... params) {
			ArrayList<String> packageName = FileUtils.readListFromFile(MyConfig.FILENAME_APP_POWERSAVE);
			ArrayList<AppInfo> allAppList = AppUtils.getLatestAppList(AppPowersaveActivity.this);
			if(allAppList == null){
				return null;
			}
			if(packageName == null||packageName.isEmpty()){
				System.out.println("省电列表为空");
				appList = null;
			}else{
				System.out.println("省电列表不为空");
		...
		protected void onPostExecute(ArrayList<AppInfo> appList) {
			...
			//注册适配器
			AppListAdapter appListAdapter = new AppListAdapter(AppPowersaveActivity.this, appList);
			listView.setAdapter(appListAdapter);
			dialog.dismiss();
		}
	}
	...
	class AsyncTaskSetModel extends AsyncTask<String, Integer, String>{
		...
		protected void onPostExecute(String result) {
			if(result.equals("")){
				Toast.makeText(AppPowersaveActivity.this, "请先获取root权限", Toast.LENGTH_SHORT).show();
				return;
			}
			...
			if(state.equals("ok")){
				...
				MainActivity.tv_showmodel.setText("当前模式:\n"+ "默认模式");// 修改主界面
				Toast.makeText(AppPowersaveActivity.this, cpuModel_str+"设置成功", Toast.LENGTH_SHORT).show();
			}else{
				Toast.makeText(AppPowersaveActivity.this, cpuModel_str+"设置失败", Toast.LENGTH_SHORT).show();
			}
			////
