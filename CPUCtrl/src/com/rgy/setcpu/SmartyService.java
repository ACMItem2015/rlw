...
public class SmartyService extends Service {
	...
	ArrayList<String> performanceList, powersaveList;
	...
	public int onStartCommand(Intent intent, int flags, int startId){
		...
		//初始化极速列表和省电列表
		performanceList = FileUtils.getAppClassifyList(SmartyService.this, MyConfig.CPUMODEL_PERFORMANCE);
		powersaveList = FileUtils.getAppClassifyList(SmartyService.this, MyConfig.CPUMODEL_POWERSAVE);
		//
		System.out.println("智能调控服务已启动");
	...
	class AsyncTaskSetModel extends AsyncTask<String, Integer, String>{
		...
		protected void onPostExecute(String result) {
			if(result.equals("")){
				Toast.makeText(SmartyService.this, "请先获取root权限", Toast.LENGTH_SHORT).show();
				stopSelf();
				return;
			}
			...
			if(state.equals("ok")){
				...
				if(!model.equals(MyConfig.CPUMODEL_DEFAULT)){
					Toast.makeText(SmartyService.this, cpuModel_str+"启动成功", Toast.LENGTH_SHORT).show();
				}
			}else{
				Toast.makeText(SmartyService.this, cpuModel_str+"启动失败", Toast.LENGTH_SHORT).show();
			}
			////
	...
	class AsyncTaskCheckApp extends AsyncTask<String, Integer, String>{
		...
		protected String doInBackground(String... params) {
			...
			String packageName = AppUtils.getCurrentPk(SmartyService.this);
